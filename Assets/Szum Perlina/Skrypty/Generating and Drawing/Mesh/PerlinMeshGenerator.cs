...
public static class PerlinMeshGenerator
{
    static MeshGeometry GenerateMeshGeometry(float[,] heightMap)
    {
        ...
        int newLeftMostX = (width - 1) / -2;
        int newTopMostZ = (height - 1) / 2;
        ...
        for(int x = 0; x < width; x++)
        {
            for(int z = 0; z < height; z++)
            {
                ...
                meshGeometry.verticies[currentVerticiesIndex] = new Vector3(x+newLeftMostX, heightMap[x, z], newTopMostZ - z);
